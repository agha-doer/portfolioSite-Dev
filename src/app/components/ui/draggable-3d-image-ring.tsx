...
export function Draggable3DImageRing({
  ...
  useEffect(() => {
    const unsubscribe = rotationY.on("change", (latestRotation) => {
      if (ringRef.current) {
        ...
        // Calculate which image is front-facing
        const normalizedRotation = ((latestRotation % 360) + 360) % 360;
        const frontIndex = Math.round(normalizedRotation / angle) % images.length;
        setFrontImageIndex(frontIndex);
        
        // Update opacity based on front-facing image
        if (!isDragging.current) {
          Array.from(ringRef.current.children).forEach((imgElement, i) => {
            const isFront = i === frontIndex;
            (imgElement as HTMLElement).style.opacity = isFront ? "1" : `${hoverOpacity}`;
          });
        }
      }
    ...
    return () => unsubscribe();
  }, [rotationY, images.length, imageDistance, currentScale, angle, hoverOpacity]);
  ...
  useEffect(() => {
    ...
    // Set initial highlighting based on initial rotation
    const normalizedRotation = ((initialRotation % 360) + 360) % 360;
    const initialFrontIndex = Math.round(normalizedRotation / angle) % images.length;
    setFrontImageIndex(initialFrontIndex);
  }, [initialRotation, angle, images.length]);
  ...
  if (ringRef.current) {
    ...
    // Restore highlighting after drag ends
    const normalizedRotation = ((rotationY.get() % 360) + 360) % 360;
    const frontIndex = Math.round(normalizedRotation / angle) % images.length;
    setFrontImageIndex(frontIndex);
            ...
            {showImages && images.map((imageUrl, index) => (
              <motion.div
                ...
                onHoverEnd={() => {
                  if (isDragging.current) return;
                  if (ringRef.current) {
                    // Restore to front-facing image highlighting
                    Array.from(ringRef.current.children).forEach((imgEl, i) => {
                      const isFront = i === frontImageIndex;
                      (imgEl as HTMLElement).style.opacity = isFront ? "1" : `${hoverOpacity}`;
                    });
                  }
                }}
              />
